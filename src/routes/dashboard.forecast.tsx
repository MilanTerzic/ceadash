...
export const Route = createFileRoute("/dashboard/forecast")({
  head: () => ({ meta: [{ title: "SEEPEX Forecast — CEA Power Dashboard" }] }),
...
function ForecastPage() {
  ...
  const chartData = useMemo(() => {
    if (!data) return [];
    const histTail = data.history.slice(
      -Math.min(data.history.length, product === "da" ? 24 * 14 : 365),
    );
    const histPts = histTail.map((h) => ({
      ts: h.ts,
      t: new Date(h.ts).toLocaleString("en-GB", {
        day: "2-digit",
        month: "short",
        hour: product === "da" ? "2-digit" : undefined,
      }),
      actual: h.price,
    }));
    const fcPts = data.forecast.map((p) => ({
      ts: p.ts,
      t: new Date(p.ts).toLocaleString("en-GB", {
        day: "2-digit",
        month: "short",
        hour: product === "da" ? "2-digit" : undefined,
      }),
  ...
  return (
    <>
      <TopBar
        title="SEEPEX Forecast"
        subtitle="Multi-product cascade: DA hourly · Week ahead · Month ahead — ARIMA + EEX anchor + fundamentals"
      />
      <div className="p-6 space-y-5">
        <Panel
          title="Forecast controls"
          actions={
            <span className="text-[10px] text-muted-foreground">Last run: {lastRefresh}</span>
          }
        >
          <div className="grid md:grid-cols-6 gap-3 items-end">
            ...
            <div className="space-y-1.5">
              ...
              <Select
                value={historyPreset}
                onValueChange={(v) => setHistoryPreset(v as HistoryPreset)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <Button onClick={() => m.mutate()} disabled={m.isPending}>
              {m.isPending ? "Forecasting…" : "Run forecast"}
            </Button>
          </div>
...
type ForecastData = NonNullable<
  ReturnType<typeof useMutation<Awaited<ReturnType<typeof runForecastV2>>>>["data"]
>;
...
function ForecastResults({
  ...
  const blendedMean = fc.length
    ? fc.reduce((a, p) => a + (p.blended ?? p.forecast), 0) / fc.length
    : null;
  ...
  const eexBadge =
    data.eex.source === "live"
      ? { cls: "border-success/40 text-success", label: "EEX LIVE" }
      : data.eex.source === "cache"
        ? { cls: "border-info/40 text-info", label: "EEX CACHE" }
  ...
  return (
    <>
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        <KPI
          label="Forecast (blended avg)"
          value={fmtPrice(blendedMean)}
          sub={`${product.toUpperCase()} · ${data.loadType}`}
          accent="primary"
        />
        <KPI
          label="Latest SEEPEX actual"
          value={fmtPrice(latest?.price ?? null)}
          sub={latest ? new Date(latest.ts).toLocaleString("en-GB") : "—"}
        />
        <KPI
          label="EEX anchor"
          value={
            data.eex_anchor != null ? (
              fmtPrice(data.eex_anchor)
            ) : (
              <span className="text-muted-foreground">n/a</span>
            )
          }
          sub={
            <Badge variant="outline" className={`${eexBadge.cls} font-mono text-[10px]`}>
              {eexBadge.label}
            </Badge>
          }
          accent="info"
        ...
        <KPI
          label="Δ vs latest"
          value={
            <span className={diff == null ? "" : diff >= 0 ? "text-success" : "text-destructive"}>
        ...
        <KPI
          label="Range"
          value={trough != null && peak != null ? `${fmtNum(trough, 1)} – ${fmtNum(peak, 1)}` : "—"}
          sub="€/MWh"
        />
      </div>
      ...
      <Panel
        title={`Forecast chart — ${product === "da" ? "hourly DA" : product === "week" ? "weekly" : "monthly"} (${data.loadType})`}
      >
        <div className="h-96">
          <ResponsiveContainer>
        ...
        <div className="text-[10px] text-muted-foreground mt-2 flex gap-4 flex-wrap">
          <span>
            Weights — stat: {(data.weights.stat * 100).toFixed(0)}% · EEX:{" "}
            {(data.weights.eex * 100).toFixed(0)}% · fundamentals:{" "}
            {(data.weights.fund * 100).toFixed(0)}%
          </span>
          <span>
            Fundamental adj: {data.fundamental_adj >= 0 ? "+" : ""}
            {fmtNum(data.fundamental_adj, 2)} €/MWh
          </span>
        </div>
      </Panel>
        ...
        <Panel title="Model diagnostics">
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div>
              ...
              <div className="font-mono">{data.diagnostics?.model ?? "—"}</div>
            </div>
            <div>
              <div className="text-[10px] uppercase text-muted-foreground">
                Statistical confidence
              </div>
              <div className="font-mono">{data.diagnostics?.stat_confidence ?? "—"}</div>
            </div>
            <div>
              <div className="text-[10px] uppercase text-muted-foreground">Training points</div>
              <div className="font-mono num">{data.diagnostics?.training_points ?? "—"}</div>
            </div>
            <div>
              <div className="text-[10px] uppercase text-muted-foreground">History range</div>
              <div className="font-mono text-xs">
                {data.diagnostics?.history_from} → {data.diagnostics?.history_to}
              </div>
            </div>
            <div>
              <div className="text-[10px] uppercase text-muted-foreground">Backtest MAE</div>
              <div className="font-mono num">
                {data.diagnostics?.mae != null ? `${fmtNum(data.diagnostics.mae, 2)} €/MWh` : "—"}
              </div>
            </div>
            <div>
              <div className="text-[10px] uppercase text-muted-foreground">Backtest MAPE</div>
              <div className="font-mono num">
                {data.diagnostics?.mape != null ? `${fmtNum(data.diagnostics.mape, 1)}%` : "—"}
              </div>
            </div>
            <div className="col-span-2">
              <div className="text-[10px] uppercase text-muted-foreground mb-1">EEX status</div>
              <div className="text-xs">
                <Badge variant="outline" className={`${eexBadge.cls} font-mono text-[10px] mr-2`}>
                  {eexBadge.label}
                </Badge>
                <span className="text-muted-foreground">
                  {data.eex.reason ?? `${data.eex.prices.length} products parsed`}
                </span>
              </div>
            </div>
            {data.diagnostics?.fallback_used && (
              <div className="col-span-2 text-[11px] text-warning flex items-start gap-2">
                <AlertTriangle className="w-3.5 h-3.5 mt-0.5" /> Fallback model used due to limited
                or low-quality history.
              </div>
            )}
          </div>
        </Panel>
        ...
        <Panel title={`Backtest — forecast made on ${backtestDate}`}>
          <div className="text-sm grid md:grid-cols-3 gap-3">
            <KPI
              ...
              value={fmtPrice(backtestRow.blended ?? backtestRow.forecast)}
              sub={`for ${new Date(backtestRow.ts).toLocaleDateString("en-GB")}`}
              accent="primary"
            />
            <KPI
              label="Forecast (statistical)"
              value={fmtPrice(backtestRow.forecast)}
              sub="model only"
            />
            <KPI
              label="80% confidence band"
              value={`${fmtNum(backtestRow.lo80, 1)} – ${fmtNum(backtestRow.hi80, 1)}`}
              sub="€/MWh"
            />
          </div>
          <p className="text-[11px] text-muted-foreground mt-3">
            To verify against actuals, set the cutoff date and compare the first forecast point with
            the realised SEEPEX print for the next period.
          </p>
        </Panel>
      )}
    </>
