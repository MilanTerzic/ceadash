...
export const Route = createFileRoute("/dashboard/spreads")({
  head: () => ({ meta: [{ title: "Spreads — CEA Power Dashboard" }] }),
...
function SpreadsPage() {
  ...
  const importRows = (data?.importRoutes ?? [])
    .map((r) => {
      const srcPoints = data?.byZone?.[r.from] ?? [];
      const srcAvg = srcPoints.length
        ? srcPoints.reduce((a, p) => a + p.price, 0) / srcPoints.length
        : 0;
      const gross = rsAvg - srcAvg;
      const cap = r.cap.data.price_eur_mwh ?? 0;
      const net = gross - cap;
      const profitableHours = rs.filter(
        (p, i) => p.price - (srcPoints[i]?.price ?? 0) - cap > 0,
      ).length;
      return {
        route: `${ZONES[r.from].name} → ${ZONES[r.to].name}`,
        gross,
        cap,
        net,
        value: net * maxMW * 24,
  ...
  const exportRows = (data?.exportRoutes ?? [])
    .map((r) => {
      const dstPoints = data?.byZone?.[r.to] ?? [];
      const dstAvg = dstPoints.length
        ? dstPoints.reduce((a, p) => a + p.price, 0) / dstPoints.length
        : 0;
      const gross = dstAvg - rsAvg;
      const cap = r.cap.data.price_eur_mwh ?? 0;
      const net = gross - cap;
      const profitableHours = rs.filter(
        (p, i) => (dstPoints[i]?.price ?? 0) - p.price - cap > 0,
      ).length;
      return {
        route: `${ZONES[r.from].name} → ${ZONES[r.to].name}`,
        gross,
      ...
      <tbody>
        {rows.map((r) => (
          <tr key={r.route} className="border-t border-border/60">
            ...
            <td className={`text-right num ${r.value > 0 ? "text-success" : ""}`}>
              {fmtEur(r.value, 0)}
            </td>
  ...
  return (
    <>
      <TopBar
        title="Spreads / Arbitrage"
        subtitle={`Tradable cap: ${maxMW} MW · gross − capacity cost`}
        onRefresh={() => q.refetch()}
            ...
            <Panel
              ...
              actions={
                <Button
                  ...
                  variant="ghost"
                  className="gap-1.5"
                  onClick={() => downloadCSV("imports.csv", importRows as never)}
                >
            ...
            <Panel
              ...
              actions={
                <Button
                  ...
                  variant="ghost"
                  className="gap-1.5"
                  onClick={() => downloadCSV("exports.csv", exportRows as never)}
                >
                  <Download className="w-3.5 h-3.5" />
                  CSV
                </Button>
