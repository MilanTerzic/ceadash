  ...
  head: () => ({
    meta: [
      { title: "Capture Prices — CEA Power Dashboard" },
      {
        name: "description",
        content:
          "Solar and wind capture prices, capture rates and negative-price exposure for Serbia.",
      },
...
function CapturePage() {
  ...
  const live = useQuery({
    ...
    queryFn: () =>
      fetchCaptureSeries({ data: { from: requestedRange.fromKey, to: requestedRange.toKey } }),
    staleTime: 60 * 60_000,
  });
  ...
  const matchedHoursInRange = useMemo(
    () => inRange.filter((p) => p.solar > 0 || p.wind > 0).length,
    [inRange],
  );
  ...
  if (live.isLoading) {
    return (
      <p className="text-sm text-muted-foreground">
        {t(
          "Fetching Serbia capture-price inputs…",
          "Učitavanje ulaznih podataka za capture price Srbije…",
        )}
      </p>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <ChartCard title={t("Methodology", "Metodologija")} description={methodologyHint}>
        <div className="grid gap-3 md:grid-cols-3 text-sm">
          ...
          <div className="rounded-xl border border-border/60 p-4">
            <div className="text-muted-foreground">
              {t("Coverage in selected range", "Pokrivenost u izabranom opsegu")}
            </div>
      ...
      {live.data?.solarSource === "modelled" && (
        <div className="rounded-2xl border border-info/40 bg-info/10 p-4 text-sm text-foreground">
          <div className="font-medium">
            {t(
              "Solar capture uses a modelled profile",
              "Solarni capture koristi modelovani profil",
            )}
          </div>
          <p className="mt-1 text-muted-foreground">
            {t(
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KpiCard
          label={t(
            "Solar output in negative-price hours",
            "Solarna proizvodnja u satima sa negativnom cenom",
          )}
          hint={t(
        ...
        <KpiCard
          label={t(
            "Wind output in negative-price hours",
            "Vetro proizvodnja u satima sa negativnom cenom",
          )}
          hint={t(
      ...
      <ChartCard
        title={t(
          "Monthly capture price vs baseload",
          "Mesečna capture cena u odnosu na baznu cenu",
        )}
        description={t(
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <ChartCard
          title={t(
            "Generation share in negative-price hours",
            "Udeo proizvodnje u negativnim satima",
          )}
          description={t(
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <ChartCard
          title={t(
            "Average hourly solar profile vs price",
            "Prosečan satni profil solarne proizvodnje u odnosu na cenu",
          )}
          description={t(
        ...
        <ChartCard
          title={t(
            "Average hourly wind profile vs price",
            "Prosečan satni profil vetro proizvodnje u odnosu na cenu",
          )}
          description={t(
      ...
      >
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
          ...
          <div className="rounded-xl border border-border/60 p-4">
            <div className="text-muted-foreground">
              {t("Hours with solar output", "Sati sa solarnom proizvodnjom")}
            </div>
            <div className="mt-1 text-2xl font-display">{solarHoursInRange}</div>
          </div>
          <div className="rounded-xl border border-border/60 p-4">
            <div className="text-muted-foreground">
              {t("Hours with wind output", "Sati sa vetro proizvodnjom")}
            </div>
...
function MonthlyCaptureTable({
  rows,
  rangeLabel,
}: {
  rows: MonthlyCaptureRow[];
  rangeLabel: string;
}) {
  const { t } = useLang();
  ...
  return (
    <ChartCard
      title={t(
        "Monthly capture, negative-price exposure and BESS spreads",
        "Mesečni capture, izloženost negativnim cenama i BESS spread-ovi",
      )}
      description={t(
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="text-left text-muted-foreground border-b border-border/60">
              ...
              <th className="py-2 pr-3 font-medium text-right">
                {t("BESS 2h net (85%)", "BESS 2h neto (85%)")}
              </th>
              <th className="py-2 pr-3 font-medium text-right">
                {t("BESS 4h gross", "BESS 4h bruto")}
              </th>
              <th className="py-2 pr-3 font-medium text-right">
                {t("BESS 4h net (85%)", "BESS 4h neto (85%)")}
              </th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r) => {
              ...
              return (
                <tr key={r.month} className="border-b border-border/40 hover:bg-muted/20">
                  ...
                  <td className="py-1.5 pr-3 text-right tabular-nums">
                    {fmtPct(r.solarNegShare, 2)}
                  </td>
                  <td className="py-1.5 pr-3 text-right tabular-nums">
                    {fmtPct(r.windNegShare, 2)}
                  </td>
                  <td className="py-1.5 pr-3 text-right tabular-nums">
                    {fmtValue(r.bess.avgGross2)}
                  </td>
                  <td className="py-1.5 pr-3 text-right tabular-nums">
                    {fmtValue(r.bess.avgNet2)}
                  </td>
                  <td className="py-1.5 pr-3 text-right tabular-nums">
                    {fmtValue(r.bess.avgGross4)}
                  </td>
                  <td className="py-1.5 pr-3 text-right tabular-nums">
                    {fmtValue(r.bess.avgNet4)}
                  </td>
                </tr>
              );
            })}
...
function DiagBlock({
  ...
  return (
    <div className="rounded-xl border border-border/60 p-4 text-sm">
      <div className="font-medium">
        {label} <span className="text-muted-foreground">({row.psrType})</span>
      </div>
      {empty ? (
        <p className="mt-1 text-muted-foreground">
          {emptyMessage}
    ...
    <details className="rounded-2xl border border-border/60 bg-card/40 p-4 text-sm">
      ...
      <div className="mt-3 space-y-3">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-x-3 gap-y-1 text-xs">
          ...
          <div className="text-muted-foreground">
            {t("SEEPEX price hours", "SEEPEX sati sa cenom")}
          </div>
          <div className="text-foreground">{priceHours ?? "—"}</div>
          <div className="text-muted-foreground">
            {t("First price ts", "Prvi vremenski žig cene")}
          </div>
          <div className="text-foreground">{fmtTs(firstPriceTs)}</div>
          <div className="text-muted-foreground">
            {t("Last price ts", "Poslednji vremenski žig cene")}
          </div>
