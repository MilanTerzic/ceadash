...
export const Route = createFileRoute("/dashboard/map")({
  head: () => ({ meta: [{ title: "Route Map — CEA Power Dashboard" }] }),
  component: MapPage,
});
...
function MapPage() {
  ...
  const rows: RouteRow[] = useMemo(() => {
    ...
    const make = (
      r: { from: ZoneCode; to: ZoneCode; label: string },
      kind: "import" | "export",
    ): RouteRow => {
    ...
    <>
      ...
      <div className="p-6 space-y-5">
        <div className="grid md:grid-cols-4 gap-4">
          <KPI
            label="RS day-avg price"
            value={fmtPrice(rsAvg)}
            accent="primary"
            source={snap.data ? "live" : "empty"}
          />
          <KPI
        ...
        <div className="grid lg:grid-cols-[1fr_340px] gap-5">
          <Panel title="Regional power flow map" actions={<Legend />}>
            <svg viewBox="0 50 720 460" className="w-full h-[560px]">
              ...
              <path
                ...
                strokeLinecap="round"
              />
              <text x="700" y="252" textAnchor="end" className="fill-info/60 text-[9px] italic">
                Danube
              </text>
          ...
          <div className="space-y-4">
            <Panel
              title={sel ? `${ZONES[sel.from].name} → ${ZONES[sel.to].name}` : "Route details"}
            >
              {sel ? (
                <div className="space-y-3 text-sm">
                  <div className="flex items-center gap-2">
                    <span
                      className={`text-[10px] font-mono uppercase tracking-wider px-1.5 py-0.5 rounded ${sel.kind === "import" ? "bg-info/15 text-info" : "bg-warning/15 text-warning"}`}
                    >
                      {sel.kind}
                    </span>
                  ...
                  <Row
                    label="Spread"
                    value={`${sel.spread > 0 ? "+" : ""}${fmtNum(sel.spread, 2)} €/MWh`}
                  />
                  ...
                  <div className="flex justify-between pt-2 border-t border-border/60">
                    ...
                    <span
                      className={`num font-bold ${sel.net > 0 ? "text-success" : "text-destructive"}`}
                    >
                      {sel.net > 0 ? "+" : ""}
                      {fmtNum(sel.net, 2)} €/MWh
                    </span>
                  </div>
                ...
                <p className="text-sm text-muted-foreground">
                  Click any line on the map to inspect price spread, capacity cost, observed flow
                  and net margin.
                </p>
              )}
            </Panel>
                ...
                {topOpportunities.map((r) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        className={`num font-semibold ${r.net > 0 ? "text-success" : "text-destructive"}`}
                      >
                        {r.net > 0 ? "+" : ""}
                        {fmtNum(r.net, 1)}
                      </span>
                ...
                .map((r) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className={r.kind === "import" ? "text-info" : "text-warning"}>
                        {r.kind}
                      </td>
                      ...
                      <td
                        className={`text-right num font-semibold ${r.net > 0 ? "text-success" : "text-destructive"}`}
                      >
                        {r.net > 0 ? "+" : ""}
                        {fmtNum(r.net, 2)}
                      </td>
  ...
  return (
    <div className="flex items-center gap-3 text-[10px] text-muted-foreground">
      <span className="flex items-center gap-1">
        <span className="w-3 h-0.5" style={{ background: "oklch(0.72 0.18 145)" }} /> profitable
      </span>
      <span className="flex items-center gap-1">
        <span className="w-3 h-0.5" style={{ background: "oklch(0.62 0.20 25)" }} /> loss
      </span>
      <span className="flex items-center gap-1">
        <span
          className="w-3 h-3 rounded-full"
          style={{
            background: "linear-gradient(90deg, oklch(0.62 0.18 220), oklch(0.62 0.18 20))",
          }}
        />
        cheap → expensive
      </span>
