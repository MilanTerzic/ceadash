...
export const Route = createFileRoute("/dashboard/capacity")({
  head: () => ({ meta: [{ title: "Capacity — CEA Power Dashboard" }] }),
...
function aggregateMonthlyRows(rows: HistoryRow[]): DisplayHistoryRow[] {
  ...
  sorted.forEach((row, idx) => {
    ...
    const segmentEnd =
      next && next.getTime() > current.getTime()
...
function CapacityPage() {
  ...
  const grouped: Record<
    string,
    { from: string; to: string; daily?: CapRow; monthly?: CapRow; annual?: CapRow }
  > = {};
  ...
  const hq = useQuery({
    ...
    queryFn: () =>
      histFn({ data: { from: hFrom_z, to: hTo_z, product, from_date: hFrom, to_date: hTo } }),
  });

  const allRows = hq.data?.rows ?? [];
  const realRows = allRows.filter(
    (r) => r.source !== "demo" && r.price_eur_mwh != null,
  ) as HistoryRow[];
  const displayRows: DisplayHistoryRow[] =
    product === "monthly" ? aggregateMonthlyRows(realRows) : realRows;
  ...
  const dateColumnLabel =
    product === "monthly" ? "Month date" : product === "annual" ? "Year date" : "Date";

  return (
    <>
      <TopBar
        title="Capacity (A25)"
        subtitle="Explicit allocation prices and volumes per border × product"
        onRefresh={() => {
          q.refetch();
          hq.refetch();
        }}
        lastRefresh={q.data?.rows?.[0]?.fetched_at}
      />
      <div className="p-6 space-y-5">
        <div className="flex items-start gap-2 text-xs text-warning bg-warning/10 border border-warning/30 rounded p-3">
          <AlertTriangle className="w-4 h-4 mt-0.5" />
          <span>
            Monthly/annual A25 prices may be published as totals (EUR) rather than EUR/MWh depending
            on the TSO. Verify before booking.
          </span>
        </div>

        <Panel
          title="Capacity prices by border"
          actions={
            <Button
              size="sm"
              variant="ghost"
              onClick={() =>
                downloadCSV(
                  ...
                  rows.map((r) => ({
                    from: r.from,
                    to: r.to,
                    annual: r.annual?.data?.price_eur_mwh,
                    monthly: r.monthly?.data?.price_eur_mwh,
                    daily: r.daily?.data?.price_eur_mwh,
                  })),
                )
              }
              className="gap-1.5"
          ...
          <table className="w-full text-sm">
            <thead className="text-[10px] uppercase tracking-wider text-muted-foreground">
              <tr>
                ...
                <th className="text-right">Daily €/MWh</th>
              ...
              {rows.map((r) => (
                <tr key={`${r.from}_${r.to}`} className="border-t border-border/60">
                  ...
                  <td className="text-right num font-medium">
                    {fmtPrice(r.daily?.data?.price_eur_mwh)}
                  </td>
                  <td className="text-right num text-muted-foreground">
                    {fmtMW(r.daily?.data?.offered_mw ?? null)}
                  </td>
                  <td className="text-right num text-muted-foreground">
                    {fmtMW(r.daily?.data?.allocated_mw ?? null)}
                  </td>
                  <td className="text-right">
                    <DataBadge source={r.daily?.source} />
                  </td>
                </tr>
              ))}
            </tbody>
              ...
              <TooltipProvider>
                <UITooltip>
                  ...
                  <TooltipContent className="max-w-xs text-xs">
                    Monthly rows are grouped by exact calendar month date. If more than one sample
                    falls in the same month, price is day-weighted inside that month.
                  </TooltipContent>
                </UITooltip>
              </TooltipProvider>
              <Button
                ...
                onClick={() =>
                  downloadCSV(
                    `capacity-history_${hFrom_z}-${hTo_z}_${product}.csv`,
                    displayRows.map((r) => ({
                      date: r.day,
        ...
        >
          ...
          {hq.isLoading ? (
            ...
            <div className="h-64 flex flex-col items-center justify-center gap-1 text-sm text-muted-foreground border border-dashed border-border rounded p-4 text-center">
              <span>
                No published ENTSO-E A25 capacity prices for {hFrom_z} → {hTo_z} ({product}) in this
                range.
              </span>
              <span className="text-xs">
                Try the opposite direction, a different product, or an earlier date range. Many SEE
                borders publish A25 only sporadically.
              </span>
            </div>
          ) : (
            <>
              {missingCount > 0 && (
                <div className="mb-3 text-[11px] text-muted-foreground">
                  {missingCount} sample{missingCount === 1 ? "" : "s"} hidden because no price was
                  available.
                </div>
              )}
              <div className="h-64">
                  ...
                  <tbody>
                    {displayRows.map((r) => (
                      <tr key={r.day} className="border-t border-border/60">
                        ...
                        <td className="pl-3 text-muted-foreground text-xs">
                          {r.unit_warning ? "EUR (verify)" : "€/MWh"}
                        </td>
                        <td className="text-right num text-muted-foreground">
                          {fmtMW(r.offered_mw)}
                        </td>
                        <td className="text-right num text-muted-foreground">
                          {fmtMW(r.allocated_mw)}
                        </td>
                        <td className="text-right">
