...
export const Route = createFileRoute("/dashboard/danube")({
  head: () => ({ meta: [{ title: "Danube — CEA Power Dashboard" }] }),
...
function DanubePage() {
  ...
  const liveRows: Row[] = Array.from(dates)
    .sort()
    .map((d) => {
  ...
  return (
    <>
      <TopBar
        title="Danube"
        subtitle="River discharge (Open-Meteo flood API, Visual Crossing fallback)"
        onRefresh={() => q.refetch()}
      />
      <div className="p-6 space-y-5">
        <Panel
          ...
          actions={
            <div className="flex items-center gap-2">
              {sources.map((s) => (
                <DataBadge key={s} source={s} />
              ))}
              <Button
                size="sm"
                variant="ghost"
                className="gap-1.5"
                onClick={() => downloadCSV("danube.csv", liveRows as never)}
              >
        ...
        >
          ...
          {!q.isLoading && liveRows.length === 0 && (
            <p className="text-xs text-muted-foreground">
              No discharge data returned for this range.
            </p>
          )}
        </Panel>
        ...
        <Panel title="Optional CSV upload (overrides auto-fetch)">
          ...
          <p className="text-xs text-muted-foreground mt-2">
            Columns: a date column plus one column per station (e.g.{" "}
            {DANUBE_STATIONS.slice(0, 3).join(", ")}…)
          </p>
        </Panel>

        {chartRows.length > 0 && (
          <Panel
            title={rows ? "Uploaded river levels by station" : "River discharge by station (m³/s)"}
          >
            <div className="h-80">
              <ResponsiveContainer>
