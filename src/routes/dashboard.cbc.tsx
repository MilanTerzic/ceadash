...
export const Route = createFileRoute("/dashboard/cbc")({
  head: () => ({ meta: [{ title: "CBC Capacity Resale — CEA Power Dashboard" }] }),
  component: CBCPage,
});
  ...
  return (
    <>
      <TopBar
        ...
        subtitle="Annual vs Monthly vs Daily — per-month resell PnL & recommendations"
      />
      <div className="p-6">
...
function arimaLikeForecast(
  history: number[],
  stepsAhead: number,
): { forecast: number | null; confidence: "low" | "medium" | "high" | "none" } {
  const xs = history.filter((v) => Number.isFinite(v));
  if (xs.length < 2)
    return { forecast: xs[xs.length - 1] ?? null, confidence: xs.length === 1 ? "low" : "none" };
  const n = xs.length;
...
function PnLBreakdown() {
    ...
    for (const pos of positions) {
      ...
      for (const r of pos.rows) {
        ...
        const pnl_m =
          fm.forecast == null
            ? null
            : (fm.forecast - pos.annual_booked_price) * pos.booked_mw * r.hours;
        const pnl_d =
          fd.forecast == null
            ? null
            : (fd.forecast - pos.annual_booked_price) * pos.booked_mw * r.hours;
        ...
        const conf =
          fm.confidence === "none" && fd.confidence === "none"
            ? "none"
            : fm.confidence === "high" || fd.confidence === "high"
              ? "high"
              : fm.confidence === "medium" || fd.confidence === "medium"
                ? "medium"
                : "low";
        recs.push({
          month: r.month,
          monthly_fcst: fm.forecast,
          daily_fcst: fd.forecast,
          pnl_m,
          pnl_d,
          rec,
          conf,
  ...
  const posSummary =
    positions.map((p) => `${p.from}→${p.to} ${fmtNum(p.booked_mw, 0)} MW`).join(", ") || "—";
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <KPI
          label="Formula"
          value={<span className="text-base font-mono">spread × MW × h</span>}
          sub="spread = selected resale − annual booked"
          accent="muted"
        />
      </div>
      ...
      {q.isLoading && (
        <Panel title="Loading…">
          <div className="text-sm text-muted-foreground py-3">Fetching positions and prices…</div>
        </Panel>
      )}
      ...
      {!q.isLoading && positions.length === 0 && (
        <Panel title="No positions">
          <div className="text-sm text-muted-foreground py-3">
            Add positions in <span className="font-medium">Manual Inputs</span> to see monthly
            resale PnL.
          </div>
        </Panel>
      )}
...
function StrategyMatrix({
  ...
  positions: PosForCard[];
  getMode: (pos: PosForCard, m: number) => SellAs;
}) {
  ...
  const badge = (m: SellAs) => {
    ...
    return (
      <Badge variant="outline" className={`${cls} text-[10px] font-mono px-1.5 py-0`}>
        {m[0].toUpperCase()}
      </Badge>
    );
  };
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs">
          <thead className="text-[10px] uppercase tracking-wider text-muted-foreground">
            <tr>
              <th className="text-left py-1.5 pr-3">Border</th>
              {months.map((m) => (
                <th key={m} className="text-center px-1">
                  {monthLabels[m - 1]}
                </th>
              ))}
            </tr>
        ...
        <div className="mt-2 text-[10px] text-muted-foreground flex gap-3 flex-wrap">
          <span>
            <Badge
              variant="outline"
              className="bg-info/15 text-info border-info/30 px-1.5 py-0 font-mono text-[10px]"
            >
              M
            </Badge>{" "}
            Monthly
          ...
          <span>
            <Badge
              variant="outline"
              className="bg-success/15 text-success border-success/30 px-1.5 py-0 font-mono text-[10px]"
            >
              D
            </Badge>{" "}
            Daily
          ...
          <span>
            <Badge
              variant="outline"
              className="bg-warning/15 text-warning border-warning/30 px-1.5 py-0 font-mono text-[10px]"
            >
              A
            </Badge>{" "}
            Annual
...
interface PosForCard {
  ...
  annual_current_price: number | null;
...
type Rec = {
  month: number;
  monthly_fcst: number | null;
  daily_fcst: number | null;
  pnl_m: number | null;
  pnl_d: number | null;
  rec: "monthly" | "daily" | "none";
...
function PositionBreakdownCard({
  ...
  const hasFutureRec = rowsResolved.some(
    (r) => r.isFuture && r.rec && r.rec.rec !== "none" && !r.mode,
  );
        ...
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div>
            ...
            <div className="text-xs text-muted-foreground mt-0.5">
              {pos.from} → {pos.to} · {fmtNum(pos.booked_mw, 1)} MW booked annual capacity · annual
              ref {fmtPrice(pos.annual_booked_price)}
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Badge
              variant="outline"
              className={`font-mono text-[10px] ${subTotal >= 0 ? "border-success/40 text-success" : "border-destructive/40 text-destructive"}`}
            >
              {fmtEur(subTotal, 0)}
            </Badge>
            {hasFutureRec && (
            ...
            <Button
              ...
              variant="ghost"
              className="gap-1.5"
              onClick={() => downloadCSV(`pnl-${pos.from}-${pos.to}.csv`, csvRows as never)}
            >
    ...
    >
      {missing > 0 && (
        <div className="flex items-start gap-2 text-[11px] text-warning bg-warning/10 border border-warning/30 rounded p-2 mb-2">
          ...
          <span>
            {missing} of {rowsResolved.length} months have no PnL — strategy not selected or
        ...
        <table className="w-full text-sm">
              ...
              return (
                <tr key={`${r.year}-${r.month}`} className="border-t border-border/60">
                  ...
                  <td className="pl-2">
                    <Select
                      value={r.mode || "unset"}
                      onValueChange={(v) => setMode(r.month, v === "unset" ? "" : (v as SellAs))}
                    >
                  ...
                  <td
                    className={`text-right num ${r.monthly_price == null ? "text-muted-foreground" : ""}`}
                  >
                    {r.monthly_price == null ? "—" : fmtNum(r.monthly_price, 4)}
                  </td>
                  <td
                    className={`text-right num ${r.daily_price == null ? "text-muted-foreground" : ""}`}
                  >
                  ...
                  <td className="pl-3 text-[10px]">
                    {r.isFuture && r.rec && r.rec.rec !== "none" ? (
                      <PredictorCell
                        r={r.rec}
                        onApply={() => setMode(r.month, r.rec!.rec as SellAs)}
                        active={r.mode === r.rec.rec}
                      />
                    ) : r.isFuture ? (
                      <span className="text-muted-foreground/60">Insufficient data</span>
                    ) : (
          ...
          <tfoot>
            <tr className="border-t border-border">
              <td
                colSpan={7}
                className="py-2 text-right text-xs uppercase tracking-wider text-muted-foreground"
              >
                Subtotal
              </td>
              <td
                className={`text-right num font-semibold ${subTotal >= 0 ? "text-success" : "text-destructive"}`}
              >
                {fmtEur(subTotal, 0)}
              </td>
              <td></td>
...
function PredictorCell({ r, onApply, active }: { r: Rec; onApply: () => void; active: boolean }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <Badge variant="outline" className={`${confCls} font-mono text-[9px] px-1 py-0`}>
        {r.conf}
      </Badge>
      {delta != null && <span className="text-muted-foreground/70">Δ {fmtEur(delta, 0)}</span>}
      {!active && (
        <button onClick={onApply} className="text-[10px] text-primary hover:underline">
          apply
        </button>
      )}
    </div>
...
function Comparison() {
  ...
  return (
    <Panel
      title="Annual vs Monthly vs Daily prices"
      actions={
        <Button
          size="sm"
          variant="ghost"
          className="gap-1.5"
          onClick={() => downloadCSV("cbc-comparison.csv", rows as never)}
        >
          <Download className="w-3.5 h-3.5" />
          CSV
        </Button>
    ...
    >
      <table className="w-full text-sm">
        <thead className="text-[10px] uppercase tracking-wider text-muted-foreground">
          <tr>
            <th className="text-left py-1.5">Border</th>
            <th className="text-right">Annual</th>
            <th className="text-right">Monthly</th>
            <th className="text-right">Daily</th>
            <th className="text-right">Δ Daily−Monthly</th>
            <th></th>
          </tr>
        </thead>
        <tbody>
          {rows.map((r) => {
            ...
            return (
              <tr key={`${r.from}_${r.to}`} className="border-t border-border/60">
                ...
                <td
                  className={`text-right num font-medium ${delta > 0 ? "text-success" : "text-destructive"}`}
                >
                  {fmtPrice(delta)}
                </td>
                <td className="text-right space-x-1">
                  {Object.entries(r.sources).map(([k, v]) => (
                    <DataBadge key={k} source={v} />
                  ))}
                </td>
              </tr>
            );
    ...
    <div className="space-y-4">
      <Panel title="Resale Strategy Predictor (ARIMA)">
        <p className="text-xs text-muted-foreground">
          ...
          Recommends the strategy with the higher expected PnL = (forecast − annual booked) × MW ×
          hours. Confidence reflects historical point count (≥12 high, ≥6 medium, &lt;6 low).
        </p>
      </Panel>
      {positions.map((pos) => {
        const histM: number[] = [];
        ...
        let step = 1;
        const future = pos.rows
          .filter((r) => r.year * 100 + r.month >= todayKey)
          .map((r) => {
            const fm = arimaLikeForecast(histM, step);
            const fd = arimaLikeForecast(histD, step);
            step++;
          ...
          <Panel key={pos.position_id} title={`${pos.label} · forecast`}>
              ...
              <tbody>
                {future.map(({ r, fm, fd, pnl_m, pnl_d, rec, conf }) => {
                  ...
                  return (
                    <tr key={`${r.year}-${r.month}`} className="border-t border-border/60">
                      <td className="py-1.5">
                        {monthLabels[r.month - 1]} {r.year}
                      </td>
                      <td className="text-right num">
                        {fm.forecast == null ? "—" : fmtNum(fm.forecast, 4)}
                      </td>
                      <td className="text-right num">
                        {fd.forecast == null ? "—" : fmtNum(fd.forecast, 4)}
                      </td>
                      <td className="text-right num">{fmtPrice(pos.annual_booked_price)}</td>
                      <td
                        className={`text-right num ${(pnl_m ?? 0) >= 0 ? "text-success" : "text-destructive"}`}
                      >
                        {pnl_m == null ? "—" : fmtEur(pnl_m, 0)}
                      </td>
                      <td
                        className={`text-right num ${(pnl_d ?? 0) >= 0 ? "text-success" : "text-destructive"}`}
                      >
                        {pnl_d == null ? "—" : fmtEur(pnl_d, 0)}
                      </td>
                      <td>
                        <Badge variant="outline" className={`${recCls} font-mono text-[10px]`}>
                          {rec === "none" ? "INSUFFICIENT DATA" : rec.toUpperCase()}
                        </Badge>
                      </td>
                      <td>
                        <Badge variant="outline" className={`${confCls} font-mono text-[10px]`}>
                          {conf}
                        </Badge>
                      </td>
                    </tr>
                ...
                {future.length === 0 && (
                  <tr>
                    <td colSpan={8} className="text-sm text-muted-foreground py-3">
                      No future months in the selected date range.
                    </td>
                  </tr>
                )}
              </tbody>
            ...
            {histM.length < 6 && histD.length < 6 && (
              <p className="text-[11px] text-warning mt-2">
                Fallback forecast used due to limited historical data ({histM.length}M /{" "}
                {histD.length}D points).
              </p>
            )}
          </Panel>
        );
...
const DEFAULT_POSITIONS = [
  {
    position_name: "HR → BA annual 2026",
    border_from: "HR",
    border_to: "BA",
    product_type: "annual",
    booked_mw: 15,
    annual_booked_price: 0.35,
    start_date: "2026-01-01",
    end_date: "2026-12-31",
    fees: 0,
    preferred_resale_mode: "auto",
    notes: "Seeded default",
  },
  {
    position_name: "BA → ME annual 2026",
    border_from: "BA",
    border_to: "ME",
    product_type: "annual",
    booked_mw: 5,
    annual_booked_price: 3.44,
    start_date: "2026-01-01",
    end_date: "2026-12-31",
    fees: 0,
    preferred_resale_mode: "auto",
...
interface Position {
  ...
  border_to: string;
  product_type: string;
  booked_mw: number | string;
  annual_booked_price: number | string;
  start_date: string;
  end_date: string;
  fees: number | string;
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>{existing ? "Edit position" : "New position"}</DialogTitle>
        </DialogHeader>
        <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <div className="space-y-1.5">
            <Label>From</Label>
            <Input
              value={form.border_from}
              onChange={(e) => setForm({ ...form, border_from: e.target.value.toUpperCase() })}
            />
          </div>
          <div className="space-y-1.5">
            <Label>To</Label>
            <Input
              value={form.border_to}
              onChange={(e) => setForm({ ...form, border_to: e.target.value.toUpperCase() })}
            />
          </div>
          <div className="space-y-1.5">
            <Label>Product</Label>
          ...
          <div className="space-y-1.5">
            <Label>Booked MW</Label>
            <Input
              value={form.booked_mw}
              onChange={(e) => setForm({ ...form, booked_mw: e.target.value })}
            />
          </div>
          <div className="space-y-1.5">
            <Label>Booked price €/MWh</Label>
            <Input
              value={form.annual_booked_price}
              onChange={(e) => setForm({ ...form, annual_booked_price: e.target.value })}
            />
          </div>
          <div className="space-y-1.5">
            <Label>Fees €</Label>
          ...
          <div className="col-span-2 space-y-1.5">
            <Label>Preferred resale</Label>
            <Select
              value={form.preferred_resale_mode}
              onValueChange={(v) => setForm({ ...form, preferred_resale_mode: v })}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="auto">Auto (best PnL)</SelectItem>
                <SelectItem value="monthly">Monthly</SelectItem>
                <SelectItem value="daily">Daily</SelectItem>
        ...
        <DialogFooter>
          <Button onClick={save}>{existing ? "Update" : "Create"}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
    ...
    <div className="grid md:grid-cols-2 gap-4">
        ...
        <table className="w-full text-xs">
          <thead className="text-[10px] uppercase tracking-wider text-muted-foreground">
            <tr>
              ...
              <th className="text-right">Monthly €</th>
              <th className="text-right">Daily €</th>
              <th className="text-right">PnL M</th>
              <th className="text-right">PnL D</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r) => (
              <tr key={r.position_id} className="border-t border-border/60">
                ...
                <td
                  className={`text-right num ${(r.pnl_monthly ?? 0) > 0 ? "text-success" : "text-destructive"}`}
                >
                  {fmtEur(r.pnl_monthly, 0)}
                </td>
                <td
                  className={`text-right num ${(r.pnl_daily ?? 0) > 0 ? "text-success" : "text-destructive"}`}
                >
                  {fmtEur(r.pnl_daily, 0)}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <Panel title="Notes">
        <p className="text-sm text-muted-foreground">
          Monthly & annual A25 prices from ENTSO-E may be published as totals (EUR) rather than
          EUR/MWh depending on TSO — verify before booking. The Resale PnL tab fetches the latest
          monthly / daily / annual prices per month from ENTSO-E with caching.
        </p>
      </Panel>
    </div>
  );
