...
export const Route = createFileRoute("/dashboard/utilization")({
  head: () => ({ meta: [{ title: "Utilization — CEA Power Dashboard" }] }),
      ...
      <div className="p-6 space-y-5">
          ...
          actions={
            <div className="flex gap-2 items-center">
              ...
              <Button
                ...
                variant="ghost"
                className="gap-1.5"
                onClick={() => downloadCSV("utilization.csv", sorted as never)}
              >
        ...
        >
          <p className="text-xs text-muted-foreground mb-3">
            Each bar = one directional border (e.g. HU → RS). Height ={" "}
            {mode === "avg" ? "average" : "peak hourly"} physical flow divided by the technical NTC
            for that direction. Red ≥ 85% = persistent congestion, amber 60–85% = heavy use, teal
            30–60% = healthy commercial use, grey &lt; 30% = under-used capacity (potential resale
            opportunity).
          </p>
          <div className="h-96">
            <ResponsiveContainer>
              <BarChart data={chartData} margin={{ left: 8, right: 16, top: 8, bottom: 60 }}>
                ...
                <Bar dataKey="utilPct" radius={[3, 3, 0, 0]}>
                  {chartData.map((d, i) => (
                    <Cell key={i} fill={pctColor(d.utilPct / 100)} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        <Panel title="Per-border detail">
              ...
              {sorted.map((r) => (
                <tr key={r.label} className="border-t border-border/60">
                  ...
                  <td
                    ...
                    style={{ color: pctColor(r.utilization_avg) }}
                  >
                    {fmtPct(r.utilization_avg)}
                  </td>
                  <td
                    className="text-right num font-semibold"
                    style={{ color: pctColor(r.utilization_peak) }}
                  >
                    {fmtPct(r.utilization_peak)}
                  </td>
              ...
              {!sorted.length && (
                <tr>
                  <td colSpan={8} className="py-6 text-center text-muted-foreground">
                    No flow data for the selected range.
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <p className="text-[10px] text-muted-foreground mt-3">
            ...
            Flows are ENTSO-E A11 hourly physical flows. Utilization &gt; 100% can occur on
            counter-flows when unscheduled loop flows exceed nominal NTC.
          </p>
        </Panel>
      </div>
    </>
