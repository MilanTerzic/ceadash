...
export function demoHourlyPrices(
  zone: ZoneCode,
  dayISO: string,
): Array<{ ts: string; price: number }> {
  ...
  for (let h = 0; h < 24; h++) {
    ...
    const peak =
      22 * Math.sin(((h - 6) / 24) * Math.PI * 2) + 14 * Math.sin(((h - 17) / 24) * Math.PI * 4);
...
export function demoCapacityPrice(
  from: ZoneCode,
  to: ZoneCode,
  product: "annual" | "monthly" | "daily",
  dayISO: string,
) {
