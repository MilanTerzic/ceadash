...
function backtest(history: number[]): { mae: number; mape: number } | null {
  ...
  for (let i = 0; i < test.length; i++) {
    ...
    if (Math.abs(test[i]) > 0.1) {
      pct += Math.abs((test[i] - fc[i]) / test[i]);
      n++;
    }
  }
...
export function forecastPrices(
  ...
  if (series.length < 48) {
    ...
    const ts0 = history.length
      ? new Date(history[history.length - 1].ts).getTime() + 3600_000
      : Date.now();
    return {
      model: "rolling_mean",
  ...
  let fc: number[];
  let resid_std = stdev(series) * 0.5;
  if (series.length >= SEASON + 48) {
    ...
    fc = seasonalNaive(series, horizon_h);
    warnings.push("History < 8 days, AR fallback to seasonal naive.");
  } else {
    model = "rolling_mean";
    fc = rollingMean(series, horizon_h);
