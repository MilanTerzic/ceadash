...
export function blend(args: {
  ...
  const wStatBase =
    args.statConfidence === "high" ? 0.5 : args.statConfidence === "medium" ? 0.4 : 0.3;
  ...
  const blended = args.statistical.map(
    (v) => wStat * v + wEex * (args.eexAnchor ?? v) + wFund * (v + args.fundamentalAdj),
  );
