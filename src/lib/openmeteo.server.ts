...
async function fetchWeatherVisualCrossing(
  lat: number,
  lon: number,
  dayISO: string,
...
export async function fetchWeatherRange(
  ...
  try {
    ...
    const data: WeatherPoint[] = h.time.map((t, i) => ({
    ...
    try {
      ...
      return {
        data: [],
        source: "demo",
        reason: `${primary instanceof Error ? primary.message : "err"} / ${fb instanceof Error ? fb.message : "err"}`,
      };
    }
  }
}
...
export async function fetchRiverDischarge(
  ...
  try {
    ...
    const data: DischargePoint[] = d.time.map((t, i) => ({
      date: t,
      discharge_m3s: (d.river_discharge[i] ?? 0) as number,
    }));
    ...
    try {
      ...
      const j = (await fetchJson(url)) as {
        days?: Array<{ datetime: string; precip: number | null }>;
      };
      const days = j.days ?? [];
      const data: DischargePoint[] = days.map((d) => ({
        date: d.datetime,
        discharge_m3s: (d.precip ?? 0) as number,
      }));
